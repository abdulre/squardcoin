...
source ./ci/squardcoin/matrix.sh
...
cd build-ci/squardcoin-$BUILD_TARGET
...
if [ "$DIRECT_WINE_EXEC_TESTS" = "true" ]; then
  ...
  wine ./src/test/test_squardcoin.exe
else
  if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
    ./src/test/test_squardcoin --catch_system_errors=no -l test_suite
  else
