...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # self.nodes[1].sendrawtransaction(txs[1]['hex']) # disabled, no RBF in SquardCoin
        #self.nodes[0].sendrawtransaction(txs[1]['hex'])  # sending on both nodes is faster than waiting for propagation # disabled, no RBF in SquardCoin
        self.sync_all()

        self.log.info("Test getbalance and getbalances.mine.untrusted_pending with conflicted unconfirmed inputs")
        # test_balances(fee_node_1=Decimal('0.02')) # disabled, no RBF in SquardCoin
