...
def squardcoinhash(s):
    return squardcoin_hash.getPoWHash(s)
...
# Objects that map to squardcoind objects, which can be serialized/deserialized
...
class CBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(squardcoinhash(r))
            self.hash = squardcoinhash(r)[::-1].hex()
...
class CompressibleBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(squardcoinhash(r))
            self.hash = int(squardcoinhash(r)[::-1].hex(), 16)
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in squardcoind indicates these should be deserialized as blocks
