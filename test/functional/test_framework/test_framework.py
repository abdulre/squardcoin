...
TMPDIR_PREFIX = "squardcoin_func_test_"
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave squardcoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop squardcoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use squardcoin-cli instead of RPC for all commands")
        parser.add_argument("--squardcoind-arg", dest="squardcoind_extra_args", default=[], action="append",
                            help="Pass extra args to all squardcoind instances")
    ...
    def setup(self):
        ...
        fname_bitcoind = os.path.join(
            ...
            "src",
            "squardcoind" + config["environment"]["EXEEXT"]
        )
        fname_bitcoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "squardcoin-cli" + config["environment"]["EXEEXT"]
        )
        self.options.bitcoind = os.getenv("BITCOIND", default=fname_bitcoind)
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=fname_bitcoincli)

        self.extra_args_from_options = self.options.squardcoind_extra_args
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: squardcoinds were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'squardcoind', self.options.bitcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'squardcoin-cli', self.options.bitcoincli) for v in versions]
    ...
    def dynamically_initialize_datadir(self, node_p2p_port, node_rpc_port):
        ...
        with open(os.path.join(new_data_dir, "squardcoin.conf"), 'w', encoding='utf8') as f:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as squardcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in squardcoin.conf
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if squardcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("squardcoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if squardcoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("squardcoin-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if squardcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("squardcoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether squardcoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether squardcoin-wallet was compiled."""
...
class SquardCoinTestFramework(BitcoinTestFramework):
    def set_test_params(self):
    ...
    def set_squardcoin_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False, evo_count=0):
        ...
        self.set_squardcoin_dip8_activation(200)
        ...
        # This is nRequestTimeout in squardcoin-q-recovery thread
    ...
    def set_squardcoin_dip8_activation(self, activate_after_block):
    ...
    def set_squardcoin_llmq_test_params(self, llmq_size, llmq_threshold):
