...
"""Class for squardcoind node under test"""
...
class TestNode():
    """A class for representing a squardcoind node under test.
    ...
    def __init__(self, i, datadir, extra_args_from_options, *, chain, rpchost, timewait, timeout_factor, bitcoind, bitcoin_cli, mocktime, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None):
        ...
        self.chain = chain
        self.bitcoinconf = os.path.join(self.datadir, "squardcoin.conf")
    ...
    def __del__(self):
        # Ensure that we don't leave any squardcoind processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time squardcoind is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by squardcoind, and
        ...
        self.running = True
        self.log.debug("squardcoind started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the squardcoind process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'squardcoind exited with status {} during initialization'.format(self.process.returncode)))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; squardcoind is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to squardcoind after {}s".format(self.rpc_timeout))
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to squardcoind
        expected_msg: regex that stderr should match when squardcoind fails

        Will throw if squardcoind starts without an error.
        Will throw if an expected_msg is provided and it does not match squardcoind's stdout."""
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
             tempfile.NamedTemporaryFile(dir=self.stdout_dir, delete=False) as log_stdout:
            try:
                self.start(extra_args, stdout=log_stdout, stderr=log_stderr, *args, **kwargs)
                ret = self.process.wait(timeout=self.rpc_timeout)
                self.log.debug(self._node_msg(f'squardcoind exited with status {ret} during initialization'))
                self.running = False
                self.process = None
                ...
                self.running = False
                self.process = None
                assert_msg = f'squardcoind should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to squardcoin-cli for an individual node"""
    def __init__(self, binary, datadir):
        self.options = []
        self.binary = binary
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.squardcoincli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with squardcoin-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run squardcoin-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running squardcoin-cli {}".format(p_args[2:]))
