...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x500=12500 SQRD) in its wallet. Using them is much more
...
with the squardcoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a squardcoind.  `P2PInterface`
...
Basic code to support P2P connectivity to a squardcoind.
