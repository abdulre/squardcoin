...
Verify that a squardcoind node can load multiple wallet files
"""
...
class MultiWalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        if self.options.is_sqlite_only:
            exp_stderr = r"Error: SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another squardcoind?"
        else:
        ...
        if self.options.is_sqlite_only:
            assert_raises_rpc_error(-4, "Wallet file verification failed. SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another squardcoind?", self.nodes[0].loadwallet, wallet_names[0])
        else:
        ...
        if self.options.is_sqlite_only:
            exp_stderr = "SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another squardcoind?"
        else:
