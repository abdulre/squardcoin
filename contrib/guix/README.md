# Bootstrappable SquardCoin Core Builds

This directory contains the files necessary to perform bootstrappable SquardCoin
Core builds.
...
To build SquardCoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      SquardCoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/squardcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/squardcoinpay/guix.sigs).
