...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_squardcoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_squardcoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_squardcoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(2).indexOf(QString("URI: squardcoin:")) != -1);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_squardcoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
