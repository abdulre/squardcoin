...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in squardcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in squardcoin.qrc)
...
static const char* qt_argv = "squardcoin-qt";
...
void BitcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success)
    {
        ...
        // squardcoin: URIs or payment requests:
        if (paymentServer) {
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"SquardCoin Core - \"").translated, ArgsManager::ALLOW_ANY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(squardcoin);
    Q_INIT_RESOURCE(squardcoin_locale);
    ...
    /// 6. Determine availability of data directory and parse squardcoin.conf
    ...
    // squardcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
