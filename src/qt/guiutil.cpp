...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool fAllowURI)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SquardCoin address (e.g. %1)").arg(
...
void setupAppearance(QWidget* parent, OptionsModel* model)
{
    if (!QSettings().value("fAppearanceSetupDone", false).toBool()) {
        ...
        dlg.setWindowIcon(QIcon(":icons/squardcoin"));
...
{
    // return if URI is not valid or is no squardcoin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::SQRD, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    QString ret = QString("squardcoin:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SQRD, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open squardcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "SquardCoin Core.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "SquardCoin Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("SquardCoin Core (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "SquardCoin Core*.lnk"
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("squardcoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a squardcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=SquardCoin Core\n";
        else
            optionFile << strprintf("Name=SquardCoin Core (%s)\n", chain);
...
void disableMacFocusRect(const QWidget* w)
{
    ...
    for (const auto& c : w->findChildren<QWidget*>()) {
        if (c->testAttribute(Qt::WA_MacShowFocusRect)) {
            c->setAttribute(Qt::WA_MacShowFocusRect, !squardcoinThemeActive());
...
void updateMacFocusRects()
{
    ...
    while (it != setRectsDisabled.end()) {
        if (allWidgets.contains(*it)) {
            (*it)->setAttribute(Qt::WA_MacShowFocusRect, !squardcoinThemeActive());
            ++it;
        } else {
