...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-generate", strprintf("Generate blocks immediately, equivalent to RPC generatenewaddress followed by RPC generatetoaddress. Optional positional integer arguments are number of blocks to generate (default: %s) and maximum iterations to try (default: %s), equivalent to RPC generatetoaddress nblocks and maxtries arguments. Example: squardcoin-cli -generate 4 1000", DEFAULT_NBLOCKS, DEFAULT_MAX_TRIES), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to squardcoind). This changes the RPC endpoint used, e.g. http://127.0.0.1:23531/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  squardcoin-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                "or:     squardcoin-cli [options] -named <command> [name=value]...  Send command to " PACKAGE_NAME " (with named arguments)\n"
                "or:     squardcoin-cli [options] help                List commands\n"
                "or:     squardcoin-cli [options] help <command>      Get help for a command\n";
...
private:
    ...
    uint8_t m_details_level{0};      //!< Optional user-supplied arg to set squardcoinboard details level
    ...
    const UniValue NetinfoHelp()
    {
        return std::string{
            "-netinfo level|\"help\" \n\n"
            "Returns a network peer connections squardcoinboard with information from the remote server.\n"
            ...
            "Suggestion: use with the Linux watch(1) command for a live squardcoinboard; see example below.\n\n"
            "Arguments:\n"
            "1. level (integer 0-4, optional)  Specify the info level of the peers squardcoinboard (default 0):\n"
            "                                  0 - Connection counts and local addresses\n"
            "                                  1 - Like 0 but with a peers listing (without address or version columns)\n"
            "                                  2 - Like 1 but with an address column\n"
            "                                  3 - Like 1 but with a version column\n"
            "                                  4 - Like 1 but with both address and version columns\n"
            "2. help (string \"help\", optional) Print this help documentation instead of the squardcoinboard.\n\n"
            "Result:\n\n"
            ...
            "> squardcoin-cli -netinfo\n\n"
            "Compact peers listing\n"
            "> squardcoin-cli -netinfo 1\n\n"
            "Full squardcoinboard\n"
            "> squardcoin-cli -netinfo 4\n\n"
            "Full live squardcoinboard, adjust --interval or --no-title as needed (Linux)\n"
            "> watch --interval 1 --no-title squardcoin-cli -netinfo 4\n\n"
            "See this help\n"
            "> squardcoin-cli -netinfo help\n"};
    }
...
public:
    ...
    UniValue ProcessReply(const UniValue& batch_in) override
    {
        ...
        if (networkinfo["version"].get_int() < 200000) {
            throw std::runtime_error("-netinfo requires squardcoind server to be running v20.0 and up");
        }
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::optional<std::string>& rpcwallet = {})
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\nMake sure the squardcoind server is running and that you are connecting to the correct RPC port.", host, port, responseErrorMessage));
...
static void ParseError(const UniValue& error, std::string& strPrint, int& nRet)
{
    if (error.isObject()) {
        ...
        if (err_code.isNum() && err_code.get_int() == RPC_WALLET_NOT_SPECIFIED) {
            strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to squardcoin-cli command line.";
        }
    } else {
