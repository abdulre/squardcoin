...
// Copyright (c) 2025 The SquardCoin Core developers
...
static const uint32_t WALLET_SQRD_KB_TO_DUFF_B = COIN / 1000; // 1 duff / B = 0.00001 SQRD / kB
...
static void SetFeeEstimateMode(const CWallet* pwallet, CCoinControl& cc, const UniValue& estimate_mode, const UniValue& estimate_param)
{
    ...
    if (cc.m_fee_mode == FeeEstimateMode::SQRD_KB || cc.m_fee_mode == FeeEstimateMode::DUFF_B) {
        ...
        if (cc.m_fee_mode == FeeEstimateMode::DUFF_B) {
            fee_rate /= WALLET_SQRD_KB_TO_DUFF_B;
        }
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    RPCHelpMan{"getnewaddress",
        ...
        RPCResult{
    RPCResult::Type::STR, "address", "The new squardcoin address"
        },
        RPCExamples{
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    RPCHelpMan{"getrawchangeaddress",
        "\nReturns a new SquardCoin address, for receiving change.\n"
...
static UniValue setlabel(const JSONRPCRequest& request)
{
    RPCHelpMan{"setlabel",
        ...
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The squardcoin address to be associated with a label."},
...
static UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{"sendtoaddress",
        ...
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The squardcoin address to send to."},
    ...
    "                             The recipient will receive less amount of SquardCoin than you enter in the amount field."},
        ...
        RPCResult{
            RPCResult::Type::ARR, "", "",
            {
                {RPCResult::Type::ARR, "", "",
                {
                    {RPCResult::Type::ARR, "", "",
                    {
                        {RPCResult::Type::STR, "address", "The squardcoin address"},
...
static UniValue listaddressbalances(const JSONRPCRequest& request)
{
    RPCHelpMan{"listaddressbalances",
        ...
        RPCResult{
            RPCResult::Type::ARR, "", "",
            {
                {RPCResult::Type::STR_AMOUNT, "amount", "The squardcoin address and the amount in " + CURRENCY_UNIT},
            }
        },
        RPCExamples{
...
static UniValue signmessage(const JSONRPCRequest& request)
{
    RPCHelpMan{"signmessage",
        "\nSign a message with the private key of an address" +
                HELP_REQUIRING_PASSPHRASE,
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The squardcoin address to use for the private key."},
...
static UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    RPCHelpMan{"getreceivedbyaddress",
        ...
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The squardcoin address for transactions."},
...
static UniValue sendmany(const JSONRPCRequest& request)
{
    RPCHelpMan{"sendmany",
                        ...
                        HELP_REQUIRING_PASSPHRASE,
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ, RPCArg::Optional::NO, "The addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The squardcoin address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
            ...
            "                           Those recipients will receive less squardcoin than you enter in their corresponding amount field.\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid SquardCoin address: ") + name_);
        }
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    RPCHelpMan{"addmultisigaddress",
        ...
        "If 'label' is specified, assign address to that label.\n",
        {
            ...
            {"keys", RPCArg::Type::ARR, RPCArg::Optional::NO, "The squardcoin addresses or hex-encoded public keys",
                {
                    {"key", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "squardcoin address or hex-encoded public key"},
                },
                },
        ...
        RPCResult{
            RPCResult::Type::ARR, "", "",
            {
                {RPCResult::Type::OBJ, "", "", Cat(Cat<std::vector<RPCResult>>(
                    {
                        {RPCResult::Type::BOOL, "involvesWatchonly", "Only returns true if imported addresses were involved in transaction"},
                        {RPCResult::Type::STR, "address", "The squardcoin address of the transaction. Not present for\n"
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::ARR, "transactions", "",
                {
                    ...
                    {
                        {RPCResult::Type::BOOL, "involvesWatchonly", "Only returns true if imported addresses were involved in transaction"},
                        {RPCResult::Type::STR, "address", "The squardcoin address of the transaction."},
                        {RPCResult::Type::STR, "category", "The transaction category.\n"
                            "\"send\"                  Transactions sent.\n"
                            "\"coinjoin\"              Transactions sent using CoinJoin funds.\n"
                            "\"receive\"               Non-coinbase transactions received.\n"
                    ...
                    {RPCResult::Type::ARR, "details", "",
                        {
                            {RPCResult::Type::OBJ, "", "",
                                {
                                    {RPCResult::Type::BOOL, "involvesWatchonly", "Only returns true if imported addresses were involved in transaction"},
                                    {RPCResult::Type::STR, "address", "The squardcoin address involved in the transaction."},
                                    {RPCResult::Type::STR, "category", "The transaction category.\n"
                                        "\"send\"                  Transactions sent.\n"
                                        "\"coinjoin\"              Transactions sent using CoinJoin funds.\n"
                                        "\"receive\"               Non-coinbase transactions received.\n"
...
static UniValue walletpassphrase(const JSONRPCRequest& request)
{
    RPCHelpMan{"walletpassphrase",
        ...
        "This is needed prior to performing transactions related to private keys such as sending squardcoin\n"
        "\nNote:\n"
...
static UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    "\nNow set the passphrase to use the wallet, such as for signing or sending squardcoin\n"
...
static UniValue lockunspent(const JSONRPCRequest& request)
{
    RPCHelpMan{"lockunspent",
        ...
        "A locked transaction output will not be chosen by automatic coin selection, when spending squardcoin.\n"
...
static UniValue loadwallet(const JSONRPCRequest& request)
{
    RPCHelpMan{"loadwallet",
        ...
        "\nNote that all wallet command-line options used when starting squardcoind will be"
...
static UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{"listunspent",
        ...
        "Optionally filter to only include txouts paid to specified addresses.\n",
        {
            ...
            {"addresses", RPCArg::Type::ARR, /* default */ "empty array", "The squardcoin addresses to filter",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "squardcoin address"},
                },
            },
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid SquardCoin address: ") + input.get_str());
            }
    ...
    if (!options.isNull()) {
      if (options.type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid squardcoin address");
            }
    ...
    RPCHelpMan{"fundrawtransaction",
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            {"changeAddress", RPCArg::Type::STR, /* default */ "pool address", "The squardcoin address to receive the change"},
...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    RPCHelpMan{"getaddressinfo",
        "\nReturn information about the given squardcoin address.\n"
        "Some of the information will only be present if the address is in the active wallet.\n",
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The squardcoin address for which to get information."},
        },
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::STR, "address", "The squardcoin address validated."},
        ...
        "Implements the Creator and Updater roles.\n",
        {
            ...
            {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "The outputs (key-value pairs), where none of the keys are duplicated.\n"
                    ...
                    "                             accepted as second parameter.",
                {
                    {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the squardcoin address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                        },
                        },
                    {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                        {
            ...
            {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                {
                    {"changeAddress", RPCArg::Type::STR_HEX, /* default */ "pool address", "The squardcoin address to receive the change"},
                    {"changePosition", RPCArg::Type::NUM, /* default */ "random", "The index of the change output"},
                    {"includeWatching", RPCArg::Type::BOOL, /* default */ "true for watch-only wallets, otherwise false", "Also select inputs which are watch only"},
                    {"lockUnspents", RPCArg::Type::BOOL, /* default */ "false", "Lock selected unspent outputs"},
                    {"feeRate", RPCArg::Type::AMOUNT, /* default */ "not set: makes wallet determine the fee", "Set a specific fee rate in " + CURRENCY_UNIT + "/kB"},
