...
static bool WalletAppInit(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        std::string strUsage = strprintf("%s squardcoin-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n";
            if (!gArgs.IsArgSet("-version")) {
                strUsage += "\n"
                    "squardcoin-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n"
                    "By default squardcoin-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n"
                    "To change the target wallet, use the -datadir, -wallet and -testnet/-regtest arguments.\n\n"
                    "Usage:\n"
                    "  squardcoin-wallet [options] <command>\n";
...
int main(int argc, char* argv[])
{
    ...
    if (method.empty()) {
        tfm::format(std::cerr, "No method provided. Run `squardcoin-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
