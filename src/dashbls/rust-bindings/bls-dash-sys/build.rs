...
fn main() {
    ...
    let bls_squardcoin_build_path = root_path.join("build");
    let bls_squardcoin_src_path = root_path.join("src");
    let c_bindings_path = root_path.join("rust-bindings/bls-dash-sys/c-bindings");

    println!("root {}", root_path.display());
    println!("bls_squardcoin_build_path {}", bls_squardcoin_build_path.display());
    println!("bls_squardcoin_src_path {}", bls_squardcoin_src_path.display());
    ...
    if bls_squardcoin_build_path.exists() {
        fs::remove_dir_all(&bls_squardcoin_build_path).expect("can't clean build directory");
    }

    fs::create_dir_all(&bls_squardcoin_build_path).expect("can't create build directory");

    let cmake_output = create_cross_cmake_command()
        .current_dir(&bls_squardcoin_build_path)
    ...
    let include_paths_file_path = bls_squardcoin_build_path.join("include_paths.txt");
    ...
    include_paths.extend([
        bls_squardcoin_build_path.join("_deps/relic-src/include"),
        bls_squardcoin_build_path.join("_deps/relic-build/include"),
        bls_squardcoin_build_path.join("src"),
        root_path.join("include/dashbls"),
        bls_squardcoin_build_path.join("depends/relic/include"),
        bls_squardcoin_build_path.join("depends/mimalloc/include"),
        root_path.join("depends/relic/include"),
        root_path.join("depends/mimalloc/include"),
        bls_squardcoin_src_path.clone(),
    ]);
    ...
    //     bls_squardcoin_build_path.join("_deps/sodium-build").display()
    // );
    ...
    println!(
        "cargo:rustc-link-search={}",
        bls_squardcoin_build_path.join("src").display()
    );

    println!("cargo:rustc-link-lib=static=dashbls");

    // Link GMP if exists
    let gmp_libraries_file_path = bls_squardcoin_build_path.join("gmp_libraries.txt");
    ...
    println!("cargo:rerun-if-changed={}", bls_squardcoin_src_path.display());
}
...
//     let bls_squardcoin_build_path = root_path.join("build");
//     let bls_squardcoin_src_path = root_path.join("src");
//     let artefacts_path = bls_squardcoin_build_path.join("artefacts");
...
//     println!("cargo:rustc-link-lib=static=sodium");
//     println!("cargo:rustc-link-lib=static=relic_s");
//     println!("cargo:rustc-link-search={}", bls_squardcoin_build_path.join("src").display());
...
fn main() {
    ...
    let bls_squardcoin_build_path = root_path.join("build");
    let bls_squardcoin_src_path = root_path.join("src");
    let bls_squardcoin_src_include_path = root_path.join("include/dashbls");
    let c_bindings_path = root_path.join("rust-bindings/bls-dash-sys/c-bindings");
    let artefacts_path = bls_squardcoin_build_path.join("artefacts");
    let target_path = artefacts_path.join(&target);
    ...
    include_paths.extend([
        bls_squardcoin_build_path.join(format!("relic-{}-{}/_deps/relic-src/include", platform, arch)),
        bls_squardcoin_build_path.join(format!("relic-{}-{}/_deps/relic-build/include", platform, arch)),
        bls_squardcoin_build_path.join("contrib/relic/src"),
        root_path.join("src"),
        root_path.join("include/dashbls"),
        root_path.join("depends/relic/include"),
        root_path.join("depends/mimalloc/include"),
        ...
        bls_squardcoin_src_path.clone(),
        bls_squardcoin_src_include_path.clone()
    ]);
    ...
    // println!("cargo:rustc-link-lib=static=relic_s");
    println!("cargo:rustc-link-lib=static=bls");
    println!("cargo:rustc-link-search={}", bls_squardcoin_src_path.display());
    println!("cargo:rustc-link-lib=static=dashbls");
    println!("cargo:rerun-if-changed={}", bls_squardcoin_src_path.display());
}
