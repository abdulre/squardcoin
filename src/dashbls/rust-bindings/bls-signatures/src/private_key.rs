...
impl PrivateKey {
    ...
    pub fn generate_squardcoin(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "squardcoin_helpers")]
    pub fn generate_squardcoin_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_squardcoin(rng))
            .collect()
    }
