...
const char * const BITCOIN_CONF_FILENAME = "squardcoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //squardcoin-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\SquardCoin
    // macOS: ~/Library/Application Support/SquardCoin
    // Unix-like: ~/.squardcoin
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/SquardCoin";
#else
    // Unix-like
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty squardcoin.conf if it does not exist
