 ...
 */
static const char* BITCOIN_PID_FILENAME = "squardcoind.pid";
...
void SetupServerArgs(NodeContext& node)
{
    ...
    argsman.AddArg("-sporkaddr=<squardcoinaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
    ...
    argsman.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by SquardCoin Platform, to the specified username.", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODE);
...
/** Sanity checks
 *  Ensure that SquardCoin Core is running in a usable environment with all
...
{
    // Make sure only a single SquardCoin Core process is using the data directory.
...
bool AppInitMain(const CoreContext& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !fs::path(args.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if SquardCoin Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if SquardCoin Core is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 7d: Setup other SquardCoin services
    ...
    // ********************************************************* Step 10a: schedule SquardCoin-specific tasks
