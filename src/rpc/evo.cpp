    ...
    static const std::map<std::string, RPCArg> mapParamHelp = {
        {"collateralAddress",
            {"collateralAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The squardcoin address to send the collateral to."}
        },
        {"collateralHash",
        ...
        {"ownerAddress",
            {"ownerAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The squardcoin address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress_register",
            {"payoutAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The squardcoin address to use for masternode reward payments."}
        },
        ...
        {"platformP2PPort",
            {"platformP2PPort", RPCArg::Type::NUM, RPCArg::Optional::NO,
                "TCP port of SquardCoin Platform peer-to-peer communication between nodes (network byte order)."}
        },
        {"platformHTTPPort",
...
static void protx_register_fund_help(const JSONRPCRequest& request, bool legacy)
{
    ...
    RPCHelpMan{rpc_full_name,
        "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 1000 SquardCoin\n"
...
static void protx_register_fund_evo_help(const JSONRPCRequest& request)
{
    RPCHelpMan{
        ...
        "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 4000 SquardCoin\n"
        "to the address specified by collateralAddress and will then function as the collateral of your\n"
        "EvoNode.\n"
        "A few of the limitations you see in the arguments are temporary and might be lifted after DIP3\n"
        "is fully deployed.\n" +
                                              ...
                                              const MnType mnType)
{
    ...
    if (!request.params[paramIdx + 6].isNull()) {
        ...
        if (!IsValidDestination(fundDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid SquardCoin address: ") + request.params[paramIdx + 6].get_str());
    }
...
static UniValue protx_update_service_common_wrapper(const JSONRPCRequest& request, const ChainstateManager& chainman, const MnType mnType)
{
    ...
    if (!request.params[paramIdx + 1].isNull()) {
        ...
        if (!IsValidDestination(feeSource))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid SquardCoin address: ") + request.params[paramIdx + 1].get_str());
    } else {
...
static UniValue protx_update_registrar_wrapper(const JSONRPCRequest& request, const ChainstateManager& chainman, const bool specific_legacy_bls_scheme)
{
    ...
    if (!request.params[4].isNull()) {
        ...
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid SquardCoin address: ") + request.params[4].get_str());
    }
...
static UniValue protx_revoke(const JSONRPCRequest& request, const ChainstateManager& chainman)
{
    ...
    if (!request.params[3].isNull()) {
        ...
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid SquardCoin address: ") + request.params[3].get_str());
        FundSpecialTx(wallet.get(), tx, ptx, feeSourceDest);
