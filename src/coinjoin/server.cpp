...
/*
    ...
    allow endless transaction that would bloat SquardCoin and make it unusable. To
...
bool CCoinJoinServer::IsInputScriptSigValid(const CTxIn& txin) const
{
    ...
    if (nTxInIndex >= 0) { //might have to do this one input at a time?
        ...
        // TODO we're using amount=0 here but we should use the correct amount. This works because SquardCoin ignores the amount while signing/verifying (only used in Bitcoin/Segwit)
