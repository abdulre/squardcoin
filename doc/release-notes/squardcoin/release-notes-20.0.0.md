# SquardCoin Core version v20.0.0
...
Currently, 10% of block rewards are set aside for the SquardCoin DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking SquardCoin in a
credit pool. This credit pool mirrors the available credit supply in SquardCoin
Platform. When a certain amount of SquardCoin is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on SquardCoin
Platform. Masternode identities receive the fees and can convert them back to
SquardCoin by withdrawing them from SquardCoin Platform.
...
- `bestCLHeightDiff` (`uint32`) is equal to the diff in heights from the mined
  block height
- `creditPoolBalance` (`int64`) is equal to all the Squish locked in all previous
  blocks minus all the Squish unlocked in all previous blocks plus all the block
...
### Removal of reject network messages from SquardCoin Core (BIP61)

This feature has been disabled by default since SquardCoin Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since SquardCoin Core version 20.0.0 there are extra changes:
...
- It is possible to run SquardCoin Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `SQRD/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from SquardCoin Core (BIP61)_](#removal-of-reject-network-messages-from-squardcoin-core-bip61) above for
...
### SquardCoin-specific changes in existing RPCs:
...
- The `squardcoin-cli -getinfo` command now displays the wallet name and balance for
...
Please check `Help -> Command-line options` in Qt wallet or `squardcoind --help` for
more information.
...
python test framework and `squardcoin-cli` tool both take advantage of this, so
for example:

```sh
squardcoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
squardcoin-cli -named createwallet mywallet load_on_startup=1
```
...
with SquardCoin’s product needs, such as SegWit and RBF, are excluded from our
...
- [v19.3.0](/blob/master/doc/release-notes/squardcoin/release-notes-19.3.0.md) released July/31/2023
- [v19.2.0](/blob/master/doc/release-notes/squardcoin/release-notes-19.2.0.md) released June/19/2023
...
[DIP-0023]: https://github.com/squardcoinpay/dips/blob/master/dip-0023.md
[DIP-0029]: https://github.com/squardcoinpay/dips/blob/master/dip-0029.md
...
[set-of-changes]: /compare/v19.3.0...squardcoinpay:v20.0.0
