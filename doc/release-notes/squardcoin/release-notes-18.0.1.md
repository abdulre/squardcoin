SquardCoin Core version v18.0.1
=========================
...
been added to support SquardCoin Platform. This update introduces versioning to
...
InstantSend are provided in [DIP-0022](https://github.com/squardcoinpay/dips/blob/master/dip-0022.md).

Deterministic InstantSend will be activated with the DIP0024 hard fork.

Governance
----------
Several improvements have been made to SquardCoin’s DAO governance system.
The governance proposal fee has been reduced from 5 SquardCoin to 1 SquardCoin following
...
system are provided in [DIP-0023](https://github.com/squardcoinpay/dips/blob/master/dip-0023.md).
...
`salvage` command in the `squardcoin-wallet` tool.
...
the deprecation of wallet accounts in SquardCoin v0.17 and introduction of PSBT
format.
...
- The `testnet` field in `squardcoin-cli -getinfo` has been renamed to `chain` and
...
deprecated. `size` will only be returned if `squardcoind` is started with
`-deprecatedrpc=size`.
...
---------------------------
This release introduces over 1000 updates from Bitcoin v0.18/v0.19/v0.20 as well as numerous updates from Bitcoin v0.21 and more recent versions. This includes multi-wallet support in the GUI, support for partially signed transactions (PSBT), Tor version 3 support, and a number of other updates that will benefit SquardCoin users. Bitcoin changes that do not align with SquardCoin’s product needs, such as SegWit and RBF, are excluded from our backporting. For additional detail on what’s included in Bitcoin, please refer to their release notes – v0.18, v0.19, v0.20.
...
See detailed [set of changes](/compare/v0.17.0.3...squardcoinpay:v18.0.1).
