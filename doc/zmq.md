...
The SquardCoin Core daemon can be configured to act as a trusted "border
router", implementing the squardcoin wire protocol and relay, making
...
The ZeroMQ feature in SquardCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building squardcoind:
...
For instance:
    ...
    $ squardcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/squardcoind.tx.raw \
...
These options can also be provided in squardcoin.conf.
...
From the perspective of squardcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into squardcoind directly. Furthermore, no information is
...
using. SquardCoind appends an up-counting sequence number to each
