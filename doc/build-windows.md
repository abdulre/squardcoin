...
Below are some notes on how to build SquardCoin Core for Windows.

The options known to work for building SquardCoin Core on Windows are:

* On Linux, using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Focal 20.04 is required
and is the platform used to build the SquardCoin Core Windows release binaries.
...
way. This will install to `c:\workspace\squardcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/squardcoin
