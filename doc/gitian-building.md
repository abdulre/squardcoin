...
*Setup instructions for a Gitian build of SquardCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SquardCoin
...
- [Building SquardCoin Core](#building-squardcoin-core)
...
Clone the git repositories for SquardCoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone 
git clone https://github.com/squardcoinpay/gitian.sigs.git
```
...
in the SquardCoin Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building SquardCoin Core
----------------

To build SquardCoin Core (for Linux, macOS and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the SquardCoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/squardcoin/.git/
...
For example:
```bash
URL=https://github.com/crowning-/squardcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit squardcoin=${COMMIT} --url squardcoin=${URL} ../squardcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit squardcoin=${COMMIT} --url squardcoin=${URL} ../squardcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit squardcoin=${COMMIT} --url squardcoin=${URL} ../squardcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the squardcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=focal on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../squardcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/squardcoin
SIGPATH=/some/root/path/squardcoin-detached-sigs

./bin/gbuild --url squardcoin=${BTCPATH},signature=${SIGPATH} ../squardcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/squardcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/squardcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/squardcoin-osx-build.assert
```
...
[squardcoin/gitian.sigs](https://github.com/squardcoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
