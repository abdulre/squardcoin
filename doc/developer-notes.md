...
- [Development guidelines](#development-guidelines)
    - [General SquardCoin Core](#general-squardcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
SquardCoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
SquardCoin Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_squardcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_squardcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/squardcoind -printtoconsole
...
# A coverage report will now be accessible at `./test_squardcoin.coverage/index.html`.
```
...
To profile a running squardcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep squardcoind` -- sleep 60
```
...
SquardCoin Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`squardcoind`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
- Net threads:
  ...
  - [ThreadSocketHandler (`d-net`)](https://doxygen.bitcoincore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 23532.
...
However, in open source software such as SquardCoin Core, where everyone uses
...
pay attention to for reviewers of SquardCoin Core code.

General SquardCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: SquardCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against SquardCoin Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `squardcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `squardcoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of SquardCoin Core, the wallet was always
...
- Use *invalid* SquardCoin addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
